 ...
 * 		Page 2 contains the last check digit for the UID, one uint8_t manufacturer internal data, and the lock bytes (see http://www.nxp.com/documents/data_sheet/MF0ICU1.pdf section 8.5.2)
...
public:
    ...
    MFRC522(const uint8_t chipSelectPin, const uint8_t resetPowerDownPin,
            SPIImpl *spiClass = &SPI, const SPISettings spiSettings = SPISettings(SPI_DEFAULT_FREQ/*SPI_CLOCK_DIV4*/, MSBFIRST, SPI_MODE0))
...
protected:
    ...
    // SPI communication
    SPIImpl *_spiClass; // SPI class which abstracts hardware.
